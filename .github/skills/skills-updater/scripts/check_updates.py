...
def _looks_like_sha(s: str) -> bool:
    """Return True if string looks like a git commit SHA (7-40 hex chars)."""
    import re
...
def check_skill_update(skill_name: str, marketplace: str, plugin_info: Dict, marketplaces: Dict) -> SkillInfo:
    ...
    if local_version in ["unknown", "", None]:
        ...
        # Compare by commit SHA.
        # Prefer comparing local_version directly when it looks like a SHA (e.g. folder
        # named by commit hash), because gitCommitSha may record an older tracking SHA
        # that doesn't reflect the currently installed content.
        if _looks_like_sha(local_version) and remote_commit.lower().startswith(local_version.lower()):
            status = UpdateStatus.UP_TO_DATE
        elif git_commit_sha and not compare_commit_sha(git_commit_sha, remote_commit):
            status = UpdateStatus.UP_TO_DATE
        elif git_commit_sha:
            status = UpdateStatus.UPDATE_AVAILABLE
        else:
            status = UpdateStatus.UNKNOWN_VERSION
    elif _looks_like_sha(local_version):
        # Remote fetch failed (likely API rate limiting) but local version IS a commit
        # SHA — treat as up-to-date rather than unknown.
        status = UpdateStatus.UP_TO_DATE
    else:
        status = UpdateStatus.UNKNOWN_VERSION
